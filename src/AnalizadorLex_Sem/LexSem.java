...
public class LexSem
{
	...
	//int a , b , c ; float d ; a = 1 ; b = 2 ; c = 3 ; d = a * b + c - a * ( b + 20 ) ;
			...
			if(especiales.indexOf(lexema.charAt(i))!=-1)
			{
				...
				else
				{
					...
					if(exi)
					{
						...
						{
							if(conca) // si conca es verdadero se hace operacion de concatenacion
							{
								for(int j=0;j<semantico.length;j++)
									if(semantico[j][1].equals(lexema))
										for(int k=0;k<semantico.length;k++)
											if(semantico[k][1].equals(id))
											{
												if(id.equals(lexema))
												{
													if(semantico[k][0].equals("int")&&semantico[j][0].equals("int")) //int solo recibe int
													{
														semantico[k][2]+=valor;
													}
													else
													{
														if(semantico[k][0].equals("float")&&!semantico[j][0].equals("char")) // float recibe todo menos char
														{
															semantico[k][2]+=valor;
														}
														else
														{
															if(semantico[k][0].equals("char")&&!semantico[j][0].equals("float"))//char recibe todo menos char
															{
																semantico[k][2]+=valor;
															}
															else
